...
%% Aktivasi Video
v = VideoWriter('CSAMT syn H homogen 1000.avi');
v.FrameRate = 3;
open(v);
...
%     RA_cal1 = (abs(Z_cal1).*abs(Z_cal1))./(mu*w);
%     phase_cal1 = atan2(imag(Z_cal1),real(Z_cal1))*(180/pi);
...
%     if rms_err<min_e
        figure()
        loglog(frekuensi,RA,'r.',frekuensi,RA_cal1,'k');
        %axis([1 1000 1 1000])
        xlabel('time(s)');
        ylabel('--');
    break
...
%% Pembuatan matriks Jacobi untuk parameter model terperturbasi
    [A] = jacobian_CSAMT(period,r,t,lr,lt,RA,RA_cal1);
    [U,S,V] = svd(A,0);
